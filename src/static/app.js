document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          <ul class="participants-list">
            ${details.participants.map(p => `<li>${p} <span class="delete-icon" data-email="${p}">&times;</span></li>`).join('')}
          </ul>
        `;
      ...
      // attach delete handlers after cards added
      activitiesList.querySelectorAll('.delete-icon').forEach(icon => {
        icon.addEventListener('click', async (event) => {
          ...
          try {
            ...
            // refresh activities after change
            fetchActivities();
          } catch (err) {
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        // update list to reflect new participant
        fetchActivities();
      } else {
